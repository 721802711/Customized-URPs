...
// PBR 全函数
half3 PBR(float3 view, float3 normal, float3 position, float3 albedo, float rough, float metal, float ao, float smoothness, float3 emissive, float3 bakeGI)
{
    ...
    half oneMinusReflectivity = OneMinusReflectivityMetallic(metal);
    half reflectivity = 1.0 - oneMinusReflectivity;
    half3 brdfDiffuse = albedo * oneMinusReflectivity;
    
    half3 giColor = brdfDiffuse * bakeGI;
    ...
    return directLightResult + IndirectColor + giColor;
