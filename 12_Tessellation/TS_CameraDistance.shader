Shader "B/TS_CameraDistance"
{
    Properties
    {
        ...
        _TessMinDist ("TessMinDist", Range(0, 10)) = 3
        _FadeDist ("FadeDist", Range(0, 30)) = 10
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            PatchTess PatchConstant(InputPatch<VertexOut, 3> patch, uint patchID : SV_PrimitiveID)
            {
                ...
                float3 cameraPosWS = GetCameraPositionWS(); // 获取相机位置
                float3 triVectexFactors = GetDistanceBasedTessFactor(patch[0].positionWS, patch[1].positionWS, patch[2].positionWS, cameraPosWS, _TessMinDist, _TessMinDist + _FadeDist); // 获取三角形顶点因子
