Shader "B/FlatTessellation"
{
    Properties
    {
        ...
        _EdgeFactor ("EdgeFactor", Range(1,16)) = 1 
        _InsideFactor ("InsideFactor", Range(1,16)) = 1 
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            VertexOut vert(appdata v)           
            {
                ...
                o.normalWS.xyz = TransformObjectToWorldNormal(v.normalOS.xyz); // 计算世界空间法线;
                return o;
            }
            ...
            v2f FlatTessDomain(PatchTess tessFactors, const OutputPatch<HullOut, 3> patch, float3 bary : SV_DOMAINLOCATION)
            {
                ...
                // 插值法线
                float3 interpolatedNormal = u * patch[0].normalWS + v * patch[1].normalWS + w * patch[2].normalWS;

                // 计算新的顶点位置
                float3 positionOS = patch[0].positionOS * u + patch[1].positionOS * v + patch[2].positionOS * w; 
                ...
                output.normalWS = interpolatedNormal; // 输出插值的法线
                return output;
            }
