...
Shader "E3D/Actor/PBR-Mask-AO-Glow"
{
	Properties
	{
	...
	SubShader
	{
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			//s6.Albedo = GammaToLinearSpace(tex2D( _AlbedoMap, uv_AlbedoMap )).rgb;
			...
			s6.Metallic = clamp( tex2DNode1.g * _Metallic , 0.0f, 1.0f);
			s6.Smoothness = clamp( (1.0f - tex2DNode1.b) * _Roughness, 0.0f, 1.0f );
			
			s6.Occlusion = tex2DNode1.r * _AO;
			...
			//surfResult6 = LinearToGammaSpace(surfResult6);
			...
			c.rgb = ( saturate( ( (0.0 + (( ase_lightAtten * 1.3 ) - 0.4) * (1.0 - 0.0) / (1.0 - 0.4)) * fresnelNode11 * _SideLightColor ) ) + ( tex2DNode1.a * _GlowColor ) + float4( clampResult9 , 0.0 ) ).rgb;
			c.a = 1;
			return c;
		}
