...
Shader "sw/character/ActorBlinnPhong"
{
	Properties
	{
		_AlbedoMap("AlbedoMap",2D) = "white"{}	//反照率
		//_NormalMap("NormalMap",2D) = "bump"{}//法线贴图
		_MaskMap("MaskMap（r空，g金属度,b粗糙度）",2D) = "white"{} //金属图，g通道存储金属度，b通道存储光滑度
		...
		_FresnelSize ("FresnelSize 反射区域大小", Range(0, 10)) = 3
		...
		_CustomFakeLightPower("Custom Fake Light Power",Float) = 0.9
	...
	SubShader
	{
		...
		pass
		{
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				half metallic = _Metallic;//金属度
				half roughness = _Glow;
				...
				half3 lightColor;
					...
					lightColor = _CustomFakeLightColor.rgb * _CustomFakeLightPower;//* _CustomFakeLightPower;
				#endif
				...
				fresnelCol = float3(saturate(fresnelCol.r),saturate(fresnelCol.g),saturate(fresnelCol.b));
				float4 color;				
				color.rgb =(diff + _Ambient.rgb) * albedo + fresnelCol;
				//color.rgb = lightDir;
				color.a = 1;
				return color;
			}
