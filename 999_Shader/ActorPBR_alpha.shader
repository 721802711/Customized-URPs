...
Shader "WL3/Actor/Darren/ActorPBR_CutOff"
{
	Properties
	{
		...
		_CutOff("CutOff", range(0.00,1.00)) = 0.5
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				half4 mainCol = tex2D(_AlbedoMap,i.uv); 
				half alpha = mainCol.a;
				half3 albedo = mainCol.rgb;//反照率 * _Color.rgb
				...
				color.rgb += _GlowColor * bChannel + fresnelCol;
	...
	Pass
		{
			...
			{
				v2f o;
				o.uv = v.texcoord.xy * _AlbedoMap_ST.xy + _AlbedoMap_ST.zw;
			...
			float4 frag(v2f i) : SV_TARGET
			{
				fixed4 testColor = tex2D(_AlbedoMap, i.uv);	
				//对于AlphaTest来说把看不见的片元去掉才能生成正确的阴影图
