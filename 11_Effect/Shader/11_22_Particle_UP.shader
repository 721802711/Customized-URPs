Shader "DFW/Effect/11_22_Particle_UP"
{
    Properties
    {
		//基础
		...
		//Ramp边缘渐变
		[Space(10)]
		[Toggle(_USE_RAMP)] _UseRamp("开启 Ramp 边缘渐变", Float) = 0 // 1. 增加关键字控制
		_RampTex ("Ramp Tex", 2D) = "white" { }

		//定向溶解
		[Header(Directional Dissolve)]
		[Toggle(_USE_DIR_DISSOLVE)] _UseDirDissolve("开启定向溶解", Float) = 0
		[Enum(UP, 0, DOWN, 1, LEFT, 2, RIGHT, 3)] _DissolveDir("溶解方向", Float) = 0
		_MinBorder ("最小边界 (Local)", Float) = -0.5
		_MaxBorder ("最大边界 (Local)", Float) = 0.5
		_DistanceEffect ("距离影响权重", Range(0, 1)) = 0.5
		...
		[Toggle(_USE_VERTEX_ANIM)] _USE_VERTEX_ANIM_NO("开启顶点动画", Float) = 0

       	[Enum(X, 0, Y, 1, Z, 2)] _VertexAnimAxis("动画轴向", Float) = 0
		_VertexAnimTex ("顶点动画控制贴图 (R通道控制强度)", 2D) = "white" {}

		_UseVertexAnimTex ("使用顶点动画贴图控制顶点动画", float) = 0
		_VertexAnimStrength ("顶点动画强度 (振幅)", Range(0, 1)) = 0.1
		_VertexAnimSpeed_U ("顶点动画 UV 滚动速度 (U)", float) = 0
		_VertexAnimSpeed_V ("顶点动画 UV 滚动速度 (V)", float) = 0


        [Space(10)]
        [Header(Sin Settings)]
        [Enum(X, 0, Y, 1, Z, 2)] _VertexAnimPhaseAxis("相位参考轴向", Float) = 1
        [Enum(None, 0, U, 1, V, 2)] _VertexAnimWeightUV("权重参考 UV", Float) = 2
        _VertexAnimSpeed ("Sin 频率 (速度)", Range(0, 20)) = 5
        _VertexAnimScale ("Sin 空间缩放 (缩放/频率)", Range(0, 10)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			#pragma shader_feature _USE_RAMP                    // Ramp边缘渐变
			#pragma shader_feature _USE_DIR_DISSOLVE            // 定向溶解

            #pragma shader_feature _USE_VERTEX_ANIM			  // 顶点动画
			...
			#include "HLSLEffectParticleLibraryUP.hlsl"
	...
	CustomEditor "EffectUPShaderGUI"
}
