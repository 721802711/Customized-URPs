...
public class EffectUPShaderGUI : ShaderGUI
{
    ...
    // Dissolve 定向溶解映射
    MaterialProperty UseRamp = null;
    MaterialProperty RampTex = null;
    MaterialProperty UseDirDissolve = null;
    MaterialProperty DissolveDir = null;
    MaterialProperty MinBorder = null;
    MaterialProperty MaxBorder = null;
    MaterialProperty DistanceEffect = null;

    // 顶点动画 (新增)
    MaterialProperty USE_VERTEXANIM = null;
    MaterialProperty VertexAnimTex = null;
    MaterialProperty UseVertexAnimTex = null;
    MaterialProperty VertexAnimSpeed_U = null;
    MaterialProperty VertexAnimSpeed_V = null;
    MaterialProperty VertexAnimStrength = null;
    MaterialProperty VertexAnimSpeed = null;
    MaterialProperty VertexAnimScale = null;
    MaterialProperty VertexAnimAxis = null;

    MaterialProperty VertexAnimPhaseAxis = null;
    MaterialProperty VertexAnimWeightUV = null;
    ...
    static bool _Vertex_Foldout = true; // 新增折叠栏开关
    ...
    public void FindProperties(MaterialProperty[] props)
    {
        ...
        UseRamp = FindProperty("_UseRamp", props);
        RampTex = FindProperty("_RampTex", props);
        UseDirDissolve = FindProperty("_UseDirDissolve", props);
        DissolveDir = FindProperty("_DissolveDir", props);
        MinBorder = FindProperty("_MinBorder", props);
        MaxBorder = FindProperty("_MaxBorder", props);
        DistanceEffect = FindProperty("_DistanceEffect", props);


        // 顶点动画属性 (新增)
        USE_VERTEXANIM = FindProperty("_USE_VERTEX_ANIM_NO", props, true);
        VertexAnimTex = FindProperty("_VertexAnimTex", props, true);
        UseVertexAnimTex = FindProperty("_UseVertexAnimTex", props, true);
        VertexAnimSpeed_U = FindProperty("_VertexAnimSpeed_U", props, true);
        VertexAnimSpeed_V = FindProperty("_VertexAnimSpeed_V", props, true);

        VertexAnimStrength = FindProperty("_VertexAnimStrength", props, true);
        VertexAnimSpeed = FindProperty("_VertexAnimSpeed", props, true);
        VertexAnimScale = FindProperty("_VertexAnimScale", props, true);

        VertexAnimAxis = FindProperty("_VertexAnimAxis", props, true);

        // 绑定新增枚举
        VertexAnimPhaseAxis = FindProperty("_VertexAnimPhaseAxis", props, true);
        VertexAnimWeightUV = FindProperty("_VertexAnimWeightUV", props, true);
    ...
    private static class Styles
    {
        ...
        public static GUIContent vertexAnimtex = new GUIContent("顶点动画控制贴图");
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
    {
        ...
        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
        _Vertex_Foldout = Foldout(_Vertex_Foldout, "顶点动画设置");
        if (_Vertex_Foldout)
        {
            EditorGUI.indentLevel++;
            EditorGUILayout.Space(10);
    ...
    void DissolveTex(Material mat)
    {
        ...
        if (USE_DISSOLVE.floatValue == 1)
        {
            ...
            // 随粒子Custom值溶解
            EditorGUI.BeginChangeCheck();
            bool disAlpha_ON = EditorGUILayout.Toggle("随粒子 Custom.x 溶解", DissolveAlpha.floatValue == 1);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            EditorGUI.BeginChangeCheck();
            bool ramp_ON = EditorGUILayout.Toggle("使用 Ramp 渐变贴图", UseRamp.floatValue == 1);
            if (EditorGUI.EndChangeCheck())
            {
                UseRamp.floatValue = ramp_ON ? 1 : 0;
            }

            if (ramp_ON)
            {
                mat.EnableKeyword("_USE_RAMP");
                m_MatEditor.TexturePropertySingleLine(new GUIContent("Ramp 贴图"), RampTex);
            }
            else
            {
                mat.DisableKeyword("_USE_RAMP");
            }

            EditorGUILayout.Space(5);
            // --- 核心新增：定向溶解控制 ---
            EditorGUI.BeginChangeCheck();
            bool dirDiss_ON = EditorGUILayout.Toggle("开启定向溶解", UseDirDissolve.floatValue == 1);
            if (EditorGUI.EndChangeCheck())
            {
                UseDirDissolve.floatValue = dirDiss_ON ? 1 : 0;
            }

            if (dirDiss_ON)
            {
                mat.EnableKeyword("_USE_DIR_DISSOLVE");
                m_MatEditor.ShaderProperty(DissolveDir, "溶解方向 (UP/DOWN/LEFT/RIGHT)");
                m_MatEditor.FloatProperty(MinBorder, "最小边界 (Local)");
                m_MatEditor.FloatProperty(MaxBorder, "最大边界 (Local)");
                m_MatEditor.RangeProperty(DistanceEffect, "距离梯度影响权重");
            }
            else
            {
                mat.DisableKeyword("_USE_DIR_DISSOLVE");
            }
            EditorGUI.indentLevel--;
        }
        else
    ...
    // 顶点动画 (新增)
    void VertexAnim(Material mat)
    {
        EditorGUI.indentLevel++;

        // 顶点动画开关
        EditorGUI.BeginChangeCheck();
        bool vertexAnim_ON = EditorGUILayout.Toggle("顶点动画", USE_VERTEXANIM.floatValue == 1);
        if (EditorGUI.EndChangeCheck())
        {
            USE_VERTEXANIM.floatValue = vertexAnim_ON ? 1 : 0;
        }

        EditorGUI.indentLevel--;


        if (USE_VERTEXANIM.floatValue == 1)
        {
            EditorGUI.indentLevel++;
            mat.EnableKeyword("_USE_VERTEX_ANIM");

            m_MatEditor.ShaderProperty(VertexAnimAxis, "动画影响轴向 (X/Y/Z)");
            m_MatEditor.RangeProperty(VertexAnimStrength, "动画强度 (振幅)");

            EditorGUILayout.Space(5);
            
            // 将 UseVertexAnimTex 改为开关类型
            EditorGUI.BeginChangeCheck();
            bool useTex_ON = EditorGUILayout.Toggle("启用贴图驱动 (关闭则使用 Sin)", UseVertexAnimTex.floatValue == 1);
            if (EditorGUI.EndChangeCheck())
            {
                UseVertexAnimTex.floatValue = useTex_ON ? 1 : 0;
            }

            EditorGUILayout.Space(10);

            if (UseVertexAnimTex.floatValue == 1)
            {
                m_MatEditor.TexturePropertySingleLine(Styles.vertexAnimtex, VertexAnimTex);
                m_MatEditor.FloatProperty(VertexAnimSpeed_U, "控制贴图 UV 速度 (U)");
                m_MatEditor.FloatProperty(VertexAnimSpeed_V, "控制贴图 UV 速度 (V)");
            }
            else
            {
                m_MatEditor.ShaderProperty(VertexAnimPhaseAxis, "相位参考轴向 (X/Y/Z)");
                m_MatEditor.ShaderProperty(VertexAnimWeightUV, "权重参考 UV (None/U/V)");

                m_MatEditor.RangeProperty(VertexAnimSpeed, "动画频率 (速度)");
                m_MatEditor.RangeProperty(VertexAnimScale, "动画固（缩放/频率)");
            }
            EditorGUI.indentLevel--;
        }
        else
        {
            mat.DisableKeyword("_USE_VERTEX_ANIM");
        }
