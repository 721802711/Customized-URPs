Shader "B/11_01_Effect"
{
    Properties
    {
        ...
        // 👉 新增：UV Wrap 控制 (U,V)
        [Vector2Drawer] _TexWrap ("WrapMode (U,V)", Vector) = (0,0,0,0)
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                // ✅ Repeat 情况直接交给贴图本身处理，不要 frac
                if (_TexWrap.x > 0.5) uv.x = clamp(uv.x, 0, 1);  // U=Clamp
                if (_TexWrap.y > 0.5) uv.y = clamp(uv.y, 0, 1);  // V=Clamp
